...
async function setConfigValue(key: string, value: string | number): Promise<boolean> {
    try {
        ...
        const newError = new Error(`Error: failed setting app configuration value: ${value} for key: ${key}`)
...
async function toggleTimer(): Promise<FormResult> {
    try {
        ...
        const setTimerState = await setConfigValue("TIMER_PAUSED", timerPaused === "1" ? "0" : "1")
        if (setTimerState) {
...
export async function handleAdminForm(prevState: any, formData: FormData): Promise<FormResult> {
    const session = await getSession()
