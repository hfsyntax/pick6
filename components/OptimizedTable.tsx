...
export default function OptimizedTable({
  ...
  useEffect(() => {
    ...
    //list-item height is players.length * 30 px + reg height of 50 + horizontal scrollbar height
    const additionalHeight = totalPlayers * playerCellSize + headerSize + 20
    setRowData({
  ...
  const Row = ({
    ...
    return (
      <div
        key={`row_${index}`}
        className={index === 0 ? "list-item header" : "list-item"}
        style={style}
      >
        {index === 0
          ? headers.map((header, headerIndex) => (
              <li
                ...
                className={header === "0'fers" ? "loser" : null}
              >
                {header}
              </li>
            ))
              ...
              .map((field) =>
                ...
                String(rows[index - 1][field]).includes("<br>") ? (
                  <li key={`result_players_${field}_${index - 1}`}>
                    <div
                      className={field === "loser_names" ? "loser" : "winner"}
                    >
                      {rows[index - 1][field].split("<br>").map((text, i) => (
                    ...
                    <input
                      ...
                      name={
                        field === "favorite_team"
                          ...
                          : rows[index - 1]["underdog_id"]
                      }
                      value={rows[index - 1]["game_id"]}
                    />
                    <li>{rows[index - 1][field]}</li>
                  </Fragment>
                ) : // single winner or loser
                ["winner_names", "loser_names"].includes(field) &&
                  !["NONE!!!", "ROLL-OVER!!!"].includes(
                    rows[index - 1][field]
                  ) ? (
                  <li
                    key={`result_player_${index - 1}`}
                    className={field === "loser_names" ? "loser" : null}
                  >
                    <Image
                      width={25}
                      height={25}
                      src={
                        rows[index - 1][field].split(" ")[0]
                          ? rows[index - 1][field].split(" ")[0]
                          : "/img/default.png"
                      }
                      alt="profile_picture"
                    />
                    <Link
                      href={`/profile/${rows[index - 1][field].split(" ")[2]}`}
                      draggable={false}
                    >
                      &nbsp;{rows[index - 1][field].split(" ")[1]}
                    </Link>
                  </li>
                ...
                field === "player_name" ? (
                  <Fragment key={`player_profile_${index}`}>
                    <Image
                      className="list-item-player"
                      width={50}
                      height={50}
                      src={
                        rows[index - 1]["picture_url"]
                          ? rows[index - 1]["picture_url"]
                          : "/img/default.png"
                      }
                      alt="profile_picture"
                    />
                    <li className="list-item-player">
                      <Link
                        href={`/profile/${rows[index - 1]["player_id"]}`}
                        draggable={false}
                      >
                        {rows[index - 1][field]}
                        <br />
